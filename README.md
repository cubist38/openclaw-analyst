...
- If you change model provider, model id, Telegram token, allow-list, or API
  keys after first boot, run `docker compose down -v` so OpenClaw config is
  regenerated from `.env`.
...
Re-run `bash install.sh` after prompt or workspace file changes.
