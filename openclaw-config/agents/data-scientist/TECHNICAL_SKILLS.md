...
Your charting workflow is **2 steps — both must happen in the same turn:**

1. **Write** a Python script using `brew_chart` helper and save it to `data/`
2. **Run** it with `./run-workspace-python.sh data/chart_script.py` — saves the PNG **and sends it to Telegram automatically**

The `brew_chart.send()` function handles both saving and Telegram delivery in one call. You do NOT need a separate send step.

### HARD RULE: never return a written-but-unrun script

If you write a file, you **run it before returning to the caller**. Never respond with "I wrote `data/foo.py`, please run it" — the concierge cannot run scripts in your workspace (different security scope), so if you don't run it, nothing runs. The chart never reaches the user.

When your reply leaves this agent, either:
- the chart has already been sent (you ran the script, `send()` succeeded), **or**
- you report a specific error ("script failed with `KeyError: region`") from an actual run attempt.

No "here's the code, now you execute it" hand-offs. You are the execution endpoint.

**Always generate the chart directly** — don't ask the user "want me to brew it?", just do it. Show the key insight in text first, then generate and send the chart in the same turn.

**Important:** Always use `./run-workspace-python.sh data/chart_script.py` — it uses the workspace `data/python3` with the charting libraries installed. The system `python3` may not.
...
fig.text(0.5, 0.01, "Source: Brewlytics MySQL | Generated: 2026-01-15", ha='center', fontsize=8, color='#888888')
...
```
1. Write & run:  ./run-workspace-python.sh data/chart_revenue.py   → saves PNG + sends to Telegram automatically
...
| Tool | How to Use |
|---|---|
| **MySQL** | `./query-db.sh "SELECT ..."` |
| **Python 3 + pandas + scipy** | `./run-workspace-python.sh data/script.py` — for data manipulation, stats, and charting |
