#!/usr/bin/env python3
"""
Generate a realistic Starbucks business intelligence database.
...
try:
    ...
    print("ERROR: PyYAML is required. Run: uv pip install -r requirements.txt")
    sys.exit(1)
...
SCHEMA_SQL = os.path.join(INIT_DIR, "01_schema.sql")
SEED_SQL = os.path.join(INIT_DIR, "02_seed.sql")
...
MYSQL_DATABASE = os.environ.get("MYSQL_DATABASE", "brewlytics")


class SQLFileConnection:
    def __init__(self):
        os.makedirs(INIT_DIR, exist_ok=True)
        self.schema = open(SCHEMA_SQL, "w", encoding="utf-8")
        self.seed = open(SEED_SQL, "w", encoding="utf-8")
        self.tables = []
        self.row_counts = {}
        self.last_result = []

        self.schema.write("-- Generated by generate_starbucks_db.py. Do not edit by hand.\n")
        self.schema.write("SET NAMES utf8mb4;\n")
        self.schema.write("SET FOREIGN_KEY_CHECKS=0;\n\n")
        self.seed.write("-- Generated by generate_starbucks_db.py. Do not edit by hand.\n")
        self.seed.write("SET NAMES utf8mb4;\n")
        self.seed.write("SET FOREIGN_KEY_CHECKS=0;\n\n")

    def cursor(self):
        return SQLFileCursor(self)
...
class SQLFileCursor:
    ...
    def execute(self, sql, params=None):
        ...
        if normalized.startswith("CREATE TABLE"):
            table = self._table_from_create(sql)
            self.conn.tables.append(table)
            self.conn.row_counts.setdefault(table, 0)
            self.conn.schema.write(f"DROP TABLE IF EXISTS `{table}`;\n")
            self.conn.schema.write(f"{sql.rstrip()};\n\n")
            self.conn.last_result = []
            return
        ...
        if normalized.startswith("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES"):
            self.conn.last_result = [(name,) for name in sorted(self.conn.tables)]
            return

        count_match = re.match(r"SELECT COUNT\(\*\) FROM `?([A-Za-z_][A-Za-z0-9_]*)`?", sql.strip(), re.IGNORECASE)
        if count_match:
            table = count_match.group(1)
            self.conn.last_result = [(self.conn.row_counts.get(table, 0),)]
            return

        if normalized.startswith("INSERT INTO"):
            table = self._table_from_insert(sql)
            self.conn.row_counts[table] = self.conn.row_counts.get(table, 0) + 1
            self.conn.seed.write(f"{self._render(sql, params)};\n")
            self.conn.last_result = []
            return
    ...
    def fetchone(self):
        return self.conn.last_result[0] if self.conn.last_result else None
    ...
    def _translate_sql(sql):
        ...
        sql = re.sub(r"\bREAL\b", "DECIMAL(12,2)", sql)
        return sql

    @staticmethod
    def _table_from_create(sql):
        match = re.search(r"CREATE\s+TABLE\s+`?([A-Za-z_][A-Za-z0-9_]*)`?", sql, re.IGNORECASE)
        if not match:
...
def create_db():
    ...
    # Print summary
    print(f"MySQL init files created: {SCHEMA_SQL}, {SEED_SQL}")
